...
# Setup path for imports - adapted for new structure
current_dir = os.path.dirname(os.path.abspath(__file__))
root_dir = os.path.dirname(current_dir)  # Go up one level from scripts to root
...
async def main():
    ...
    try:
        ...
        await _run_main_processing(project_logger, config_dict)
    finally:
...
async def _run_main_processing(project_logger, config_dict):
    """Main processing logic extracted for proper resource management."""
    global progress_tracker
    
    # Initialize results manager for professional output organization
    results_manager = ResultsManager(root_dir)
    ...
    if mode_choice == "1":
        ...
        try:
            ...
            from models.ensemble.core.bayesian_engine import BayesianDeepfakeEngine, BayesianConfig
    ...
    try:
        ...
        sys.path.insert(0, os.path.join(root_dir, 'inference', 'visualization'))
        from advanced_plotter import AdvancedPhysicsPlotter
        
        # Create plotter instance - use standardized directory structure
        vis_output_dir = os.path.join(root_dir, "inference", "visualization")
    ...
    if project_logger and all_results:
        try:
            ...
            print(f"  - Plots saved to: inference/visualization/")
            print(f"  - Analysis reports saved to: scripts/output/")
            print(f"  - Detailed logs saved to: scripts/logs/")
    ...
    # Generate professional results organization
    print("\n" + "="*60)
    print("ORGANIZING RESULTS")
    print("="*60)
    
    try:
        # Organize all existing outputs into professional structure
        results_manager.organize_existing_outputs()
        
        # Prepare comprehensive results data
        final_stats = progress_tracker.get_stats()
        results_data = {
            'data_overview': {
                'total_files': final_stats['total'],
                'processed_files': final_stats['processed'], 
                'successful_files': final_stats['successful'],
                'failed_files': final_stats['failed'],
                'file_types': len(set(result.get('file_type', 'unknown') for result in all_results))
            },
            'performance_metrics': {
                'processing_time_seconds': final_stats['elapsed_time'],
                'average_time_per_file': final_stats['avg_time_per_file'],
                'success_rate_percent': final_stats['success_rate'],
                'files_per_second': final_stats['processed'] / max(final_stats['elapsed_time'], 1)
            },
            'key_findings': [
                f"Successfully processed {final_stats['successful']} out of {final_stats['total']} files",
                f"Average processing time: {final_stats['avg_time_per_file']:.2f} seconds per file",
                f"Overall success rate: {final_stats['success_rate']:.1f}%"
            ],
            'recommendations': [
                "Review the interactive dashboard for detailed insights",
                "Examine statistical reports for feature discrimination analysis", 
                "Use visualizations to understand data patterns",
                "Check detailed logs for any processing issues"
            ]
        }
        
        # Add Bayesian analysis results if available
        bayesian_results = [r for r in all_results if 'bayesian_confidence' in r]
        if bayesian_results:
            avg_confidence = sum(r.get('bayesian_confidence', 0) for r in bayesian_results) / len(bayesian_results)
            results_data['performance_metrics']['average_bayesian_confidence'] = avg_confidence
            results_data['key_findings'].append(f"Bayesian analysis completed on {len(bayesian_results)} files")
        
        # Create executive summary
        summary = results_manager.create_executive_summary(results_data)
        
        # Create navigation index
        results_manager.create_index_file()
        
        session_dir = results_manager.get_session_directory()
        
        print("✓ Results successfully organized in professional structure")
        print(f"\n📁 ORGANIZED RESULTS LOCATION:")
        print(f"   Main Directory: {session_dir}")
        print(f"\n📋 QUICK ACCESS:")
        print(f"   📊 Executive Summary: {session_dir}/summary_reports/executive_summary.html")
        print(f"   📈 Visualizations: {session_dir}/visualizations/")
        print(f"   📋 Data Analysis: {session_dir}/data_analysis/")
        print(f"   🎯 Interactive Dashboard: {session_dir}/interactive_dashboards/")
        print(f"   📝 Navigation Index: {session_dir}/index.md")
        
        print(f"\n🔗 QUICK COMMANDS:")
        print(f"   Open Executive Summary: open '{session_dir}/summary_reports/executive_summary.html'")
        print(f"   Open Interactive Dashboard: open '{session_dir}/interactive_dashboards/'")
        print(f"   View Navigation Index: cat '{session_dir}/index.md'")
        
    except Exception as e:
        print(f"Warning: Could not complete results organization: {e}")
        import traceback
        traceback.print_exc()
